...
# 修改 shift_up_or_down 函数的实现（修复版）
def shift_up_or_down_prob_0_01(segment):
    """以1%的概率随机上下移调音符（修复版）"""
    shift = random.choice([-1, 1])  # 改为 -1 或 1，避免 2 导致跳度太大

    for track in segment.tracks:
        active_notes = {}  # 跟踪当前活动的音符
        current_time = 0
        ...
        for msg in track:
            ...
            if msg.type == 'note_on' and msg.velocity > 0:
                ...
                if random.random() < 0.01:
                    ...
                    # 确保移调后的音符在钢琴范围内
                    new_note = max(21, min(108, original_note + shift))
                    if new_note != original_note:
                        msg.note = new_note
                        # 记录这个音符需要移调
                        active_notes[original_note] = (current_time, new_note)
                else:
                    active_notes[msg.note] = (current_time, msg.note)

            elif msg.type == 'note_off' or (msg.type == 'note_on' and msg.velocity == 0):
                if msg.note in active_notes:
                    start_time, original_note_info = active_notes[msg.note]
                    # 如果原始音符被移调过，且这个note_off事件对应那个移调的音符
                    if isinstance(original_note_info, tuple):
                        original_note, new_note = original_note_info
                        if msg.note == original_note:
                            msg.note = new_note
                    del active_notes[msg.note]


# 同样修复 shift_up_or_down_prob_0_001 函数
def shift_up_or_down_prob_0_001(segment):
    """以0.1%的概率随机上下移调音符（修复版）"""
...
def final_adaptation(segment, **kwargs):
    ...
    for function in functions.keys():
        result += functions[function] * function(segment)
    return result / total_weight

class Iterator:
    def __init__(self):
        self.rmg = RandomMusicGenerator()
        self.segmentPoolSize = 128
    ...
    # 修改 Iterator 类的 iteratesingle 方法
    def iteratesingle(self):
        l1 = len(self.segmentPool)
        temp = self.segmentPool.copy()

        # 执行变异
        if self.mutationPool:
            print("\n变异阶段:")
            l2 = len(self.mutationPool)
            for i in tqdm(range(self.mutationIntensity), desc="变异进度"):
                n = random.randint(0, l1 - 1)
                m = random.randint(0, l2 - 1)
                # 调用变异函数
                mutation_func = self.mutationPool[m]
                mutation_func(temp[n])

        # 执行交叉
        print("\n交叉阶段:")
        l = len(temp)
        for i in tqdm(range(self.crossIntensity), desc="交叉进度"):
            x = random.randint(0, l - 1)
            y = random.randint(0, l - 1)
            n = random.randint(1, self.segmentLength - 1)
            try:
                lx, rx = self.rmg.splitMidiAtMeasure(temp[x], n)
                ly, ry = self.rmg.splitMidiAtMeasure(temp[y], n)
                temp[x] = self.rmg.concatenateMidiFiles([lx, ry])
                temp[y] = self.rmg.concatenateMidiFiles([ly, rx])
        ...
        print("\n计算适应度阶段:")
        temp_with_fitness = []
        for seg in tqdm(temp, desc="计算适应度"):
            if True:
            #try:
...
if __name__ == "__main__":
    ...
    # 初始化变异池
    iterator.mutationPool = [shift_up_or_down_prob_0_01, shift_up_or_down_prob_0_001]

    # 运行迭代
    print("开始迭代...")
    iterator.iteration(10)
    ...
    # 获取顶级段落
    segments_with_fitness = []
    for idx, segment in enumerate(iterator.segmentPool):
        fitness = iterator.adaptationFunction(segment)
        segments_with_fitness.append((fitness, idx, segment))

    segments_with_fitness.sort(reverse=True, key=lambda x: x[0])

    # 播放前5个
    for rank, (fitness, idx, segment) in enumerate(segments_with_fitness[:5], 1):
        print(f"\n播放排名 #{rank} 的音乐片段 (适应度: {fitness:.4f})...")

        # 保存临时文件用于播放
        temp_filename = f"temp_top_{rank}.mid"
        segment.save(temp_filename)
        print(f"已保存为临时文件: {temp_filename}")
        ...
        try:
            ...
            print(f"正在播放... (使用系统默认MIDI播放器)")
            time.sleep(5)  # 给播放器时间打开

        except Exception as e:
            print(f"无法自动播放: {e}")
            print("请手动使用MIDI播放器打开以下文件:")
            print(f"  {temp_filename}")

        # 询问是否继续
        if rank < 5:
            response = input("按回车继续播放下一个，或输入 'q' 停止播放: ")
            if response.lower() == 'q':
                print("停止播放")
                break
